...
output: 
  BiocStyle::html_document:
    toc_float: true
vignette: >
  %\VignetteIndexEntry{8. Upgrading existing scripts}
...
# Where to start
We documented this upgrade in a few different ways to help you upgrage existing 
...
**Note: don't run the *BEFORE* chunks with the latest RCy3 package**

## Example #1: displayGraph
...
#Note 1: the network SUID is returned rather than an R object 
#Note 2: display and layout are done automatically
```

## Example #2: loading nodeData 
*BEFORE*
```{r, eval=FALSE}
...
*AFTER*
```{r}
node.data <- data.frame(id=c('A','B','C'), 
                        moleculeType=c('kinase','TF','cytokine'),
                        lfc=c(-1.2, 1.8, 3.2),
                        stringsAsFactors = FALSE)
loadTableData(node.data, 'id')

#Note 1: simply compose a data.frame and then load it! No need to initialize or alter local graphNEL object
#Note 2: stringsAsFactors = FALSE is important or else string values won't load
#Note 3: set and display are no longer needed
```



## Example #3: setDefaults
*BEFORE*
```{r, eval=FALSE}
...
#Note 1: no more cw!
#Note 2: subtley renamed functions for consistency across package (sorry!)
```


## Example #4: Rule based mapping
*BEFORE*
```{r, eval=FALSE}
...
#Note 1: no more cw!
#Note 2: Rules are now Mappings (like they are called in Cytoscape)
#Note 3: "interpolate" (a.k.a. "continuous") is the default, so you can leave it out
```


## Example #5: Selecting nodes 
*BEFORE*
```{r, eval=FALSE}
selectNodes(cw, 'B')
...
#Note 1: no more cw!
#Note 2: selectNodes is by node SUID by default, so you now have to specify if using node name (or other alternative columns). This protects against ambiguous selection.
#Note 3: selectNodes actually returns the selected nodes and edges after the operation, so you could just do: 

nodes <- selectNodes('B', 'name')$nodes
```


## Example #6: Saving and exporting
*BEFORE*
```{r, eval=FALSE}
...
#Note 1: no more cw!
#Note 2: subtle renames to match actual operations and outputs
```
