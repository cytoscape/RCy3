...
\begin{itemize}
  \item Java SE 7. Cytoscape and CyREST currently do not support Java SE 6 or Java SE 8 anymore/yet. It can be can downloaded from Oracle here:
     \url{http://www.oracle.com/technetwork/java/javase/downloads/jre7-downloads-1880261.html}.
  \item Cytoscape version 3.1.1+, which can be downloaded from 
     \url{http://cytoscape.org}.
  \item CyREST, version 0.9.13 or later, a Cytoscape plugin which provides the Cytoscape end of the communication layer, can be downloaded from the Cytoscape plugins website:
...
Alternatively you can download the CyREST plugin 0.9.13+ as .jar file from Cytoscape's app store here: \\
...
\item Keiichiro Ono for generously providing CyREST as well as help and support for new functionalities and changes.

\item Paul Shannon for advice and help for transforming this package from using XMLRPC and CytoscapeRPC to using CyREST.
