# ------------------------------------------------------------------------------
#' @export
setNodePropertyDirect <- function(node.names, new.values, visual.property, network=NULL, base.url=.defaultBaseUrl) {
    ...
    if (length(new.values) != length(node.names)) {
        ...
        for (i in 1:length(node.SUIDs)) {
            ...
            res<- cyrestPUT(paste("networks", net.SUID, "views", view.SUID, "nodes", node.SUID,
                                  visual.property, 'bypass', sep="/"),
                            body = list(visualProperty=visual.property, value=new.value),
                            base.url = base.url)
        }
    }
}
...
#' @export
clearNodePropertyDirect <- function(node.names, visual.property, network=NULL, base.url=.defaultBaseUrl) {
    net.SUID <- getNetworkSuid(network)
    net.views.SUIDs <- getNetworkViews(net.SUID, base.url)
    view.SUID <- as.character(net.views.SUIDs[[1]])
    node.SUIDs <- .nodeNameToNodeSUID(node.names, net.SUID, base.url)
    ...
    for (i in 1:length(node.SUIDs)) {
        node.SUID <- as.character(node.SUIDs[i])
        new.value <- new.values[i]
        res<- cyrestDELETE(paste("networks", net.SUID, "views", view.SUID, "nodes", node.SUID,
                                 visual.property, 'bypass', sep="/"), base.url=base.url)
    }
}

# ------------------------------------------------------------------------------
#' @export
setEdgePropertyDirect <- function(edge.names, new.values, visual.property, network=NULL, base.url=.defaultBaseUrl) {
    net.SUID <- getNetworkSuid(network)
    net.views.SUIDs <- getNetworkViews(net.SUID, base.url)
    view.SUID <- as.character(net.views.SUIDs[[1]])
    ...
    if (length(new.values) != length(edge.names)) {
        ...
        for (i in 1:length(edge.SUIDs)) {
            ...
            res <- cyrestPUT(paste("networks", net.SUID, "views", view.SUID, "edges", edge.SUID,
                                   visual.property, 'bypass', sep="/"),
                             body = list(visualProperty=visual.property, value=current.value),
                             base.url = base.url)
        }
    }
}
...
#' @export
clearEdgePropertyDirect <- function(edge.names, visual.property, network=NULL, base.url=.defaultBaseUrl) {
    net.SUID <- getNetworkSuid(network)
    net.views.SUIDs <- getNetworkViews(net.SUID, base.url)
    view.SUID <- as.character(net.views.SUIDs[[1]])
    edge.SUIDs <- .nodeNameToNodeSUID(edge.names, net.SUID, base.url)
    
    for (i in 1:length(edge.SUIDs)) {
        edge.SUID <- as.character(edge.SUIDs[i])
        new.value <- new.values[i]
        res<- cyrestDELETE(paste("networks", net.SUID, "views", view.SUID, "nodes", edge.SUID,
                                 visual.property, 'bypass', sep="/"), base.url=base.url)
    }
}
...
#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeColorDirect <- function (node.names, new.colors, network=NULL, base.url=.defaultBaseUrl) {
    ...
    return(setNodePropertyDirect(node.names, new.colors, "NODE_FILL_COLOR", network, base.url))
}
#------------------------------------------------------------------------------------------------------------------------
# only works if node dimensions are unlocked (that is not tied together).  see lockNodeDimensions (T/F)
#' @export
setNodeSizeDirect <- function (node.names, new.sizes, network=NULL, base.url=.defaultBaseUrl) {
...
#' @export
setNodeWidthDirect <- function (node.names, new.widths, network=NULL, base.url=.defaultBaseUrl) {
    # unlock node dimensions
    lockNodeDimensions (FALSE)
    ...
    return(setNodePropertyDirect(node.names, new.widths, "NODE_WIDTH", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
# only works if node dimensions are not locked (that is, tied together).  see lockNodeDimensions (T/F)
#' @export
setNodeHeightDirect <- function (node.names, new.heights) { # Comment Tanja: Could pass in visual style here
    # unlock node dimensions
    lockNodeDimensions (FALSE)
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.heights, "NODE_HEIGHT", network, base.url))
}

# ------------------------------------------------------------------------------
#' @export
setNodeLabelDirect <- function(node.names, new.labels, network=NULL, base.url=.defaultBaseUrl) {
...
# ------------------------------------------------------------------------------
#' @export
setNodeFontSizeDirect <- function(node.names, new.sizes, network=NULL, base.url=.defaultBaseUrl) {
    size.type.errors = 0
    ...
    if(size.type.errors < 1) {
        setNodePropertyDirect(node.names, new.sizes, "NODE_LABEL_FONT_SIZE", network, base.url)
    }
}
...
#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeLabelColorDirect <- function (node.names, new.colors, network=NULL, base.url=.defaultBaseUrl) {
    for (current.color in new.colors){
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.colors, "NODE_LABEL_COLOR", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeShapeDirect <- function (node.names, new.shapes, network=NULL, base.url=.defaultBaseUrl) {
    ...
    wrong.node.shape <- sapply(unique.node.shapes, function(x) !(x %in% getNodeShapes(base.url)))
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.shapes, "NODE_SHAPE", network, base.url))
}


#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeBorderWidthDirect <- function (node.names, new.sizes, network=NULL, base.url=.defaultBaseUrl) {
    for (current.size in new.sizes){
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.sizes, "NODE_BORDER_WIDTH", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeBorderColorDirect <- function (node.names, new.colors, network=NULL, base.url=.defaultBaseUrl) {
    ...
    return(setNodePropertyDirect(node.names, new.colors, "NODE_BORDER_PAINT", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeOpacityDirect <- function (node.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    ...
    setNodePropertyDirect(node.names, new.values, "NODE_TRANSPARENCY", network, base.url)
    setNodePropertyDirect(node.names, new.values, "NODE_BORDER_TRANSPARENCY", network, base.url)
    setNodePropertyDirect(node.names, new.values, "NODE_LABEL_TRANSPARENCY", network, base.url)
}
#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeFillOpacityDirect <- function (node.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    for (current.value in new.values){
    ...
    # set the node border color direct
    return(setNodePropertyDirect(node.names, new.values, "NODE_TRANSPARENCY", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeBorderOpacityDirect <- function (node.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    for (current.value in new.values){
        # ensure the opacity value is a double and between 0 and 255
        if (! is.double(current.value) || current.value < 0  || current.value > 255) {
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.values, "NODE_BORDER_TRANSPARENCY", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
setNodeLabelOpacityDirect <- function (node.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    for (current.value in new.values){
        # ensure the opacity value is a double and between 0 and 255
        if (! is.double(current.value) || current.value < 0  || current.value > 255) {
    ...
    # set the node property direct
    return(setNodePropertyDirect(node.names, new.values, "NODE_LABEL_TRANSPARENCY", network, base.url))
}

#------------------------------------------------------------------------------------------------------------------------
#' @export
hideSelectedNodes <- function (network=NULL, base.url=.defaultBaseUrl) {
...
# ------------------------------------------------------------------------------
#' @export
hideNodes <- function(node.names, network=NULL, base.url=.defaultBaseUrl) {
    setNodePropertyDirect(node.names, 'false', "NODE_VISIBLE", network, base.url)
} 
## END hideNodes
...
# ------------------------------------------------------------------------------------------------------------------------
#' @export
unhideNodes <- function(node.names, network=NULL, base.url=.defaultBaseUrl) {
...
#------------------------------------------------------------------------------------------------------------------------
#' @export
setEdgeOpacityDirect <- function (edge.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    for (current.value in new.values){
        # ensure the opacity value is a double and between 0 and 255
        if (! is.double(current.value) || current.value < 0  || current.value > 255) {
    ...
    setEdgePropertyDirect(edge.names, new.values, "EDGE_LABEL_TRANSPARENCY", network, base.url)
    setEdgePropertyDirect(edge.names, new.values, "EDGE_TRANSPARENCY", network, base.url)
}

# ------------------------------------------------------------------------------
# Sets both edge color properties, independent on arrowColorMatchesEdge setting.
#' @export
setEdgeColorDirect <- function (edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    for (color in new.value){
        if (.isNotHexColor(color)){
            return()
        } 
    }
    setEdgePropertyDirect(edge.names, new.value, "EDGE_STROKE_UNSELECTED_PAINT", network, base.url)
    setEdgePropertyDirect(edge.names, new.value, "EDGE_UNSELECTED_PAINT", network, base.url)
}

# ------------------------------------------------------------------------------
#' @export
setEdgeLabelDirect <- function(edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
...
# ------------------------------------------------------------------------------
#' @export
setEdgeFontFaceDirect <- function(edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    setEdgePropertyDirect(edge.names, new.value, "EDGE_LABEL_FONT_FACE", network, base.url)
}
## END setEdgeFontFaceDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeFontSizeDirect <- function(edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    size.type.errors = 0
    ...
    if(size.type.errors < 1) {
        setEdgePropertyDirect(edge.names, new.value, "EDGE_LABEL_FONT_SIZE", network, base.url)
    }
}
...
#------------------------------------------------------------------------------------------------------------------------
#' @export
setEdgeLabelColorDirect <- function (edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    ...
    # set the edge property direct
    return(setEdgePropertyDirect(edge.names, new.value, "EDGE_LABEL_COLOR", network, base.url))
}
#------------------------------------------------------------------------------------------------------------------------
#' @export
setEdgeTooltipDirect <- function (edge.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    ...
    # set the edge property direct
    return(setEdgePropertyDirect(edge.names, new.values, "EDGE_TOOLTIP", network, base.url))
}

# ------------------------------------------------------------------------------
#' @export
setEdgeLineWidthDirect <- function(edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    for (current.size in new.value){
        # ensure the sizes are numbers
        if (!is.numeric(current.size)) {
...
# ------------------------------------------------------------------------------
#' @export
setEdgeLineStyleDirect <- function(edge.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    unique.new.values <- unique(new.values)
    
    wrong.values <- 
        sapply(unique.new.values, function(v) { !(toupper(v) %in% getLineStyles(base.url)) })
    ...
    return(setEdgePropertyDirect(edge.names, toupper(new.values), "EDGE_LINE_TYPE", network, base.url))
}
## END setEdgeLineStyleDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeSourceArrowShapeDirect <- function(edge.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    unique.new.values <- unique(new.values)
    
    wrong.values <- sapply(unique.new.values, function(v) { !(toupper(v) %in% getArrowShapes(base.url)) })
    ...
    # returns TRUE or FALSE if issues have been found (like invalid edges, ...)
    return(setEdgePropertyDirect(edge.names, toupper(new.values), "EDGE_SOURCE_ARROW_SHAPE", network, base.url))
}
## END setEdgeSourceArrowShapeDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeTargetArrowShapeDirect <- function (edge.names, new.values, network=NULL, base.url=.defaultBaseUrl) {
    unique.new.values <- unique(new.values)
    ...
    # returns TRUE or FALSE if issues have been found (like invalid edges, ...)
    return(setEdgePropertyDirect(edge.names, toupper(new.values), "EDGE_TARGET_ARROW_SHAPE", network, base.url))
}
## END setEdgeTargetArrowShapeDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeSourceArrowColorDirect <- function(edge.names, new.colors, network=NULL, base.url=.defaultBaseUrl) {
    for(current.color in new.colors) {
    ...
    # returns TRUE or FALSE if issues have been found (like invalid edges, ...)
    return(setEdgePropertyDirect(edge.names, new.colors, "EDGE_SOURCE_ARROW_UNSELECTED_PAINT", network, base.url))
}
## END setEdgeSourceArrowColorDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeTargetArrowColorDirect <- function(edge.names, new.colors, network=NULL, base.url=.defaultBaseUrl) {
    for(current.color in new.colors) {
        if (.isNotHexColor(current.color)){
            return()
        }
    ...
    # returns TRUE or FALSE if issues have been found (like invalid edges, ...)
    return(setEdgePropertyDirect(edge.names, new.colors, "EDGE_TARGET_ARROW_UNSELECTED_PAINT", network, base.url))
}
## END setEdgeTargetArrowColorDirect

# ------------------------------------------------------------------------------
#' @export
setEdgeLabelOpacityDirect <- function(edge.names, new.value, network=NULL, base.url=.defaultBaseUrl) {
    ...
    # returns TRUE or FALSE if issues have been found (like invalid edges, ...)
    return(setEdgePropertyDirect(edge.names, new.value, "EDGE_LABEL_TRANSPARENCY", network, base.url))
}
