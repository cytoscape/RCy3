...
#' @import httr
#' @export
doRequestRemote<-function(method, qurl, params=NULL, headers=NULL, ...){
    kwargs <- list(...) 
    if('json' %in% kwargs){
        data = kwargs$json
    }
    else if ('data' %in% kwargs){
        data = decode(kwargs$data, "UTF-8")
    }
    else {
        data = NULL
    }
    tryCatch(
        expr = {
            ...
            r <- POST(URLencode(url_post), body = http_request, encode="json", content_type_json())
            print(status_code(r))
        },
        error = function(e){
