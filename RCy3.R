...
setMethod ('createWindow', 'CytoscapeWindowClass',
  function (obj) {
    ...
    conn.str = paste(obj@uri, pluginVersion(obj), "networks",sep="/")
    res <- POST(url=conn.str, body=cygraph, encode="json")
...
setMethod ('getWindowCount', 'CytoscapeConnectionClass',
  function (obj) {
    conn.url <- paste(obj@uri,pluginVersion(obj),"networks/count",sep="/")
    
    res <- GET(conn.url)
    num.of.windows <- rawToChar(res$content)
...
setMethod ('deleteWindow',  'CytoscapeConnectionClass',
  function (obj, window.title=NA) {
    ...
    conn.str = paste(obj@uri, pluginVersion(), "networks", window.id, sep="/")
...
# ------------------------------------------------------------------------------
setMethod ('getGraph', 'CytoscapeWindowClass', function(obj) {
  return(obj@graph)
})
...
# ------------------------------------------------------------------------------
setMethod('getNodeCount', 'CytoscapeWindowClass', function (obj) {
  net.suid = as.character(obj@window.id)
  version = pluginVersion(obj)
  
  resource.uri <- paste(obj@uri, version, "networks", net.suid, "nodes/count", sep="/")
  res <- GET(resource.uri)
  count <- rawToChar(res$content)
  return(as.integer(count))
})
# ------------------------------------------------------------------------------
setMethod ('getEdgeCount', 'CytoscapeWindowClass', function (obj) {
  net.suid = as.character (obj@window.id)
  version = pluginVersion(obj)
...
# ------------------------------------------------------------------------------
setMethod('getNodeAttributeNames', 'CytoscapeConnectionClass', function(obj) {
  # plugin version
  plugin.v <- pluginVersion(obj)
  # network SUID
  net.suid <- as.character(obj@window.id)
  # construct CyREST API string
  resource.uri <- paste(obj@uri, plugin.v, "networks", net.suid, "tables/defaultnode/columns", sep="/")
  res.existing.tbl.cols <- GET(resource.uri)
  existing.tbl.cols <- unname(fromJSON(rawToChar(res.existing.tbl.cols$content)))
  existing.tbl.cols.names <- sapply(existing.tbl.cols, function(n) n$name)
  return(existing.tbl.cols.names)
})
...
# ------------------------------------------------------------------------------
setMethod('clearSelection', 'CytoscapeWindowClass', function(obj) {
  net.suid <- as.character(obj@window.id)
  version <- pluginVersion(obj)
  # get all existing SUIDs / nodes in the current Cytoscape window
  resource.uri <- paste(obj@uri, version, "networks", net.suid, "tables/defaultnode/columns/SUID", sep="/")
  cyrest.response <- GET(resource.uri)
  cywin.node.SUIDs <- unique(fromJSON(rawToChar(cyrest.response$content))[[2]])
  
  # set the all 'selected' column values to FALSE in the current Cytoscape window 
  resource.uri <- paste(obj@uri, version, "networks", net.suid, "tables/defaultnode/columns/selected", sep="/")
  unselect.node.req <- lapply(cywin.node.SUIDs, function(i) {list(SUID=i, value='false')})
  unselect.node.req.JSON = toJSON(unselect.node.req)
  unselect.node.req.res <- PUT(url=resource.uri, body=unselect.node.req.JSON, encode="json")
  
  redraw(obj)
  invisible(unselect.node.req.res)
})
...
setMethod('selectNodes', 'CytoscapeWindowClass', 
  function(obj, node.names, preserve.current.selection = TRUE) {
    ...
    select.nodes.req.JSON <- toJSON(selected.nodes)
    
    select.nodes.req.res <- PUT(url=resource.uri, body=selected.nodes.JSON, encode="json")
...
# ------------------------------------------------------------------------------
setMethod('getSelectedNodeCount', 'CytoscapeWindowClass', function(obj) {
  net.suid = as.character(obj@window.id)
  version = pluginVersion(obj)
  resource.uri <- paste(obj@uri, version, "networks", net.suid, "tables/defaultnode/columns/selected", sep="/")
  
  selected.nodes.req <- GET(resource.uri)
  
  num.selected.nodes <- sum(fromJSON(rawToChar(selected.nodes.req$content))$values)
  # CyREST has a bug: column values are returned duplicated
  # below line should be changed when the bug is resolved
  return(num.selected.nodes/2)
})
   
# ------------------------------------------------------------------------------
setMethod('getSelectedNodes', 'CytoscapeWindowClass', function(obj) {
  net.suid <- as.character(obj@window.id)
  ...
  if(getSelectedNodeCount(obj) == 0) {
    ...
    version <- pluginVersion(obj)
    # RESTful string: get all SUIDs from the current Cytoscape window
    request.uri <- paste(obj@uri, version, "networks")
