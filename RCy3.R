#modified from https://github.com/pshannon-bioc/RCy3/blob/master/R/RCy3.R

# move these to the DESCRIPIONT file
library(httr)
library(graph)
library(methods)
library(RJSONIO)
...
CytoscapeWindow = function (title, graph = new('graphNEL', edgemode = 'directed'), host = 'localhost', port = 1234,
                            create.window = TRUE, overwriteWindow = FALSE, collectTimings = FALSE)
{
    ...
    if(create.window) {
        #creates window id aka network id
...
{
    # checks if user installed correct plugin version
    ...
    expected.version = 1
    # alert user if his/her plugin version is below the require version
    if (plugin.version < expected.version) { 
        write (' ', stderr ())
        write (sprintf ('This version of the RCytoscape package requires CyREST plugin version %s or greater.', expected.version), stderr ())
...
setMethod ('ping', signature = 'CytoscapeConnectionClass',
           function (obj) { 
               api.str <- paste(obj@uri, pluginVersion(obj), sep="/")
               res <- GET(api.str)
...
setMethod ('pluginVersion', 'CytoscapeConnectionClass', 
           function (obj) {
               ...
               res <- GET(obj@uri)
               # vector with available plugin versions
...
setMethod ('createWindow', 'CytoscapeWindowClass',
           # creates a new window in Cytoscape
           function (obj) {
               # winndow.ids are often character versions of integers but they can be 
               # character titles, as when an SBML file is imported from disk.
               obj@graph@graphData$name <- obj@title
               
               graph_attr <- obj@graph@graphData
               
               el = list(nodes = list(), edges = list())
               
               cygraph <- toJSON(list(data = graph_attr, elements = el))
               
               api.str = paste(obj@uri, pluginVersion(obj), "networks",sep="/")
               res <- POST(url=api.str, body=cygraph, encode="json")
               window.id <- unname(fromJSON(rawToChar(res$content)))
...
setMethod ('getWindowCount', 'CytoscapeConnectionClass',
           # get number of networks in current session
           function (obj) {
               api.url <- paste(obj@uri,pluginVersion(obj),"networks/count",sep="/")
               
               res <- GET(api.url)
               num.of.windows <- substr(rawToChar(res$content),10,10)
               return(as.integer(num.of.windows))
           })
#------------------------------------------------------------------------------------------------------------------------
...
setMethod ('getWindowList', 'CytoscapeConnectionClass',
           # return a  named list, in which the values are the titles of the windows in the current Cytoscape Desktop.
           function (obj) {
               ...
               for(i in 1:length(result.raw)) {
                   # id must be character vector
                   #id = as.character(result.raw[[i]]$data$SUID)
                   #title = result.raw[[i]]$data$name
                   #result[[id]] = title
               } # for i
               
               #TODO2: once implemented remove the next statement
               result = result.raw
               #TODO get names for the ids end
               return(result)
...
setMethod ('getLayoutNames', 'CytoscapeConnectionClass', 
           # get the names of possible layouts
           function (obj) {
               api.str <- paste(obj@uri, pluginVersion(obj), "apply/layouts", sep="/")
...
setMethod ('getGraph', 'CytoscapeWindowClass',
           # return the bioconductor graph object which belongs to the specified CytoscapeWindow object
           function (obj) {
               return (obj@graph)
           })
...
setMethod ('sendNodes', 'CytoscapeWindowClass',
           ...
           # send nodes from R to Cytoscape
           function(obj) {
               # print number of nodes to screen
               ...
               # construct api url
               # obj@window.id is the networkid
               api.str <- paste(obj@uri, pluginVersion(obj), "networks", as.character(obj@window.id), "nodes", sep="/")
               
               # get the node names as a list
               network.nodes <- nodes(obj@graph)
               
               #TODO check if nodes exist
               
               # convert nodes to JSON format
               network.nodes.json <- toJSON(network.nodes)
               
               # post to Cytoscape
               res <- POST(url = api.str, body = network.nodes.json, encode = "json")
               
               #TODO: delete this print statement
               print(api.str)
               
               # returns SUID of the new node(s) with the name, associates returned SUIDs and the names of nodes
               # res is in the following format: e.g. list(name = "A", SUID = 147)
               
               new.nodes.list <- unname(fromJSON(rawToChar(res$content)))
               #TODO begin return the suids? or in sending nodes first request the ids
               #obj@graph@suidnamedict = new.nodes.list
               #return obj
               #TODO end return the suids?
               
               write('ending sendNodes', stderr())
               
               invisible(new.nodes.list)
           })
...
setMethod ('sendEdges', 'CytoscapeWindowClass',
           # send edges from graph in R to Cytoscape, given existing edges in R
           function (obj) {
               #print how many edges there are
               ...
               #edge table: source - target - edgeType
               ...
               #TODO problem will arise if two nodes have the same name
               #    #get node SUIDs //suids are doubled
               #     api.str <- paste(obj@uri, pluginVersion(obj), "networks", as.character(obj@window.id), "tables/defaultnode/columns/SUID", sep="/")
               #     res <- GET(url=api.str)
               #     node.suids <- unname(fromJSON(rawToChar(res$content)))
               #     node.suids <- node.suids[[2]]
               #     #remove this line once not doubled anymore
               #     node.suids[c(TRUE, FALSE)]
               
               #     #get node NAMEs //names are doubled
               #     api.str <- paste(obj@uri, pluginVersion(obj), "networks", as.character(obj@window.id), "tables/defaultnode/columns/name", sep="/")
               #     res <- GET(url=api.str)
               #     node.names <- unname(fromJSON(rawToChar(res$content)))
               #     node.names <- node.names[[2]]
               #     #remove this line once not doubled anymore
               #     node.names[c(TRUE, FALSE)]
               
               #get all rows in the nodes table
               api.str <- paste(obj@uri, pluginVersion(obj), "networks", as.character(obj@window.id), "tables/defaultnode/rows", sep="/")
               res <- GET(url=api.str)
               node.tbl.rows <- fromJSON(rawToChar(res$content))
               node.tbl.rows <- lapply(node.tbl.rows, function(row) {
                   # convert each group to a data frame and transpose it.
                   data.frame(t(matrix(unlist(row), byrow=F)))
               })
               node.tbl.rows <- do.call(rbind, node.tbl.rows)
               names(node.tbl.rows) <- c("suid", "name", "shared", "selected")
               
               #source and target edges and their type
               a = tbl.edges$source
               b = tbl.edges$target
               edge.type = tbl.edges$edgeType
               
               #convert a and b to SUIDs
               sourceA = lapply(a, FUN=function(r)subset(node.tbl.rows, name == r)$suid)
               target = lapply(b, FUN=function(r)subset(node.tbl.rows, name == r)$suid)
               
               #convert nodes to JSON format
               #old version: network.nodes.json <- toJSON(list(list(source=aSuid, target=bSuid)))
               ab <- cbind(sourceA, target)
               r <- apply(ab,MARGIN=1,FUN=function(r)list(source=unname(r[[1]]),target=unname(r[[2]])))
               network.nodes.json <- toJSON((r))
...
setMethod ('setNodeAttributes', 'CytoscapeWindowClass',
           #Transfer the named edge attribute from the the R graph (found in obj@graph) to Cytoscape.
           
           #TODO check this function again
           function (obj, attribute.name) {
               #if no nodes in graph: skip
               if (length (nodes (obj@graph)) == 0){
                   return ()
               }
               
               #set node attributes via direct function
...
setMethod ('setNodeAttributesDirect', 'CytoscapeWindowClass',
           ...
           function (obj, attribute.name, attribute.type, node.names, values) {
               ...
               # alert user if number of nodes is not the same as number of nodes with associated values
               ...
               #TODO> this is old: chad.debug (obj, "in RCytoscape::setNodeAttributesDirect")       
               #
               ...
               node.attributes.json <- toJSON(list(attributes.to.send))
               result <- POST(url=api.str, body=node.attributes.json, encode="json")
               invisible (result)
               ...
               #       #write (result, stderr ())
               #       }
               #TODO remove this old stuff END
               
               
               #TODO: question from Tanja to Georgi
               #these are the datatypes: "type":"data type, Double, String, Boolean, Long, Integer",
               # what about these three: Long, data type
               #TODO: question from Tanja to Georgi END
               ...
               #     
               # get all values
               df.values <- as.data.frame(values)
               val <- df.values[["values"]]
               print(val)
               #     # convert node names to SUIDs
               #     node.suids = lapply(node.names, FUN=function(r)subset(node.tbl.rows, name == r)$suid)
               #     
               #     # convert nodes to JSON format
               #     ab <- cbind(node.suids, val)
               #     r <- apply(ab,MARGIN=1,FUN=function(r)list(suid=unname(r[[1]]), as.character(attribute.name)=unname(r[[2]])))
               #     network.nodes.json <- toJSON((r))
               #     print(network.nodes.json)
               ...
               attribute.values.to.send <- list(values) #values
               node.attribute.values.json <- toJSON(attribute.values.to.send)
               
               #print(node.attribute.values.json)
               
               result <- PUT(url=api.str, body=node.attribute.values.json, encode="json")
               invisible (result)
...
setMethod ('setEdgeAttributes', 'CytoscapeWindowClass',
           #setEdgeAttributes: Transfer the named edge attribute from the the R graph
           #(found in obj@graph) to Cytoscape. This method is typically called by displayGraph,
           #which will suffice for most users' needs. It transfers the specified edge attributes,
           #for all edges, from the cw@graph slot to Cytoscape.
           function (obj, attribute.name) {
               if (length (edgeNames (obj@graph)) == 0){
                   return ()
               }
...
setMethod ('setEdgeAttributesDirect', 'CytoscapeWindowClass',
           # Transfer the named edge attribute, for all named edges, to Cytoscape.
           # one edge at a time
           function (obj, attribute.name, attribute.type, edge.names, values) {
               ...
               # populate columns with attribute values
               api.str <- paste(obj@uri, pluginVersion(obj), "networks", as.character(obj@window.id), "tables/defaultnode", as.character(attribute.name), sep="/")
               
               attribute.values.to.send <- list(val) #values
               node.attribute.values.json <- toJSON(attribute.values.to.send)
...
setMethod ('displayGraph', 'CytoscapeWindowClass',
           ...
           # sends nodes, edges and their attributes to Cytoscape and displays them there   
           function(obj) {
               if(length(nodes(obj@graph)) == 0) {
               ...
               #TODO change without res?
               #res <- POST(url=obj@uri, body=list, encode="json")
               
               #TODO change without res? end
...
noa = function (graph, node.attribute.name)
{
    ...
    # return NA if the nodes don't have attribute names and
    # return attribute names if the nodes have attributes
...
eda = function (graph, edge.attribute.name)
{
    ...
    # return NA if the edges don't have attribute names and
    # return attribute names if the edges have attributes
...
setMethod ('getVisualStyleNames', 'CytoscapeConnectionClass',
           ...
           function (obj) {
               api.str <- paste(obj@uri, pluginVersion(obj), "apply/styles", sep="/")
               res <- GET(url=api.str)
...
setMethod ('copyVisualStyle', 'CytoscapeConnectionClass',
           ...
           # Once you have designed a visual style, you may wish to duplicate it, perhaps in preparation
           # for adding further mapping rules. Another scenario arises when style rules have been added
           # to the 'default' style, and you wish to create a Cytoscape session file with your current
           # network and this default style. However, the default style is not saved into a session, only
           # explicitly named styles are. Use this method to achieve this.
           function (obj, from.style, to.style) {
               ...
               # get the from.style
               api.str <- paste(obj@uri, pluginVersion(obj), "styles", from.style, sep="/")
               #api.str <- paste0(api.str, ".json")
               from.style.json <- GET(url=api.str)
               #from.style.json <- toJSON(unname(fromJSON(rawToChar(from.style.json$content))))
               
               #TODO: from here on NOT YET WORKING
               # create a new visual style form JSON
...
setMethod ('setVisualStyle', 'CytoscapeConnectionClass',
           ...
           # Cytoscape provides a number of canned visual styles. You can also create your own.
           # Use this method to establish an (already-defined) visual style as the style which
           # governs the display of a network in the specified CytoscapeWindow object.
           function (obj, new.style.name) {
               
               current.names = getVisualStyleNames (obj)
               
               # alert user if he/she wants to set a style that does not exist
               ...
               #change the current style to the desired style
...
setMethod ('lockNodeDimensions', 'CytoscapeConnectionClass',
           ...
           function (obj, new.state, visual.style.name='default') {
               ...
               #TODO incorporate the statement below by doing an api call
...
setMethod ('saveImage', 'CytoscapeWindowClass',
           # get png image of a network view
           ...
           function (obj, file.name, image.type, scale=1.0) {
               ...
               # svg and pdf are currently not yet supported via the Cyrest API.
...
setMethod ('saveNetwork', 'CytoscapeWindowClass',
           # save the current session into a local file system
           # NOT YET WORKING
           function (obj, file.name, format='gml') {
               api.str <- paste(obj@uri, pluginVersion(obj), "session", sep="/")
               result <- GET(url=api.str)
               ####old:
               #id = as.character (obj@window.id)
...
toCytoscape <- function(g) {
    ...
    # Extract graph object attributes : graph_attr is 'list'
    graph_attr = g@graphData
    print(graph_attr)
    # Extract nodes : node_count is integer vector
    node_count = numNodes(g)
    print(node_count)
    # Set default ID for all graph nodes
    nodeDataDefaults(g, attr="id") <- 0
    
    g.nodes.ids <- character(0)
    
    if(exists("name", where = nodeData(g))) {
        for(i in 1:node_count) {
            g.nodes.ids <- c(g.nodes.ids, nodeData(g, n="b", attr="name")[[1]])
        }
        # print(paste("name found: ", g.nodes.ids))
    } else {
        print("NO NODE NAMES!")
        for(i in 1:node_count) {
            g.nodes.ids <- c(g.nodes.ids, as.character(i))
        }
    }
    
    nodeData(g, n = nodes(g), attr="id") <- g.nodes.ids
    
    # REMOVE BELOW LINE when deploying to production
    nodeDataDefaults(g, attr="name") <- "default"
    
    # g.nodes : vector
    nodes <- nodes(g)
    nds = list()
    
    g.node.attr = nodeData(g)
    
    v_names = names(nodeDataDefaults(g)) 
    v_names.len = length(v_names)
    
    v_attr <- vector("list", v_names.len)
    
    names(v_attr) <- v_names
    
    for(k in 1:v_names.len) {
        for(m in 1:length(g.node.attr)) {
            v_attr[[k]] <- c(v_attr[[k]], g.node.attr[[m]][[k]])
        }
    }
    ...
    edge_count <- 0
    # moved up from original script
    eds = list()
    
    if(length(edges) > 0) {
        for(i in 1:length(edges)) {
            edge_count <- edge_count + length(edges[[i]])
        }
    }
    
    if(edge_count > 0) {
        edges.matrix <- matrix(0, nrow = edge_count, ncol = 2)
        
        cur.matrix.row <- 1
        
        for(i in 1:length(edges)) {
            node.degree <- length(edges[[i]])
            if(node.degree == 0) {
                next
            }
            for(j in 1:node.degree) {
                r <- c(names(edges[i]), edges[i][[1]][[j]])
                
                edges.matrix[cur.matrix.row,] = r
                
                cur.matrix.row <- cur.matrix.row + 1
            }
        }
        
        # REMOVE THE BELOW 2 LINES when deploying to production
        edgeDataDefaults(g, "weight") <- 1
        edgeDataDefaults(g, "code") <- "plain"
        # list
        g.edge.attr <- edgeData(g)
        
        e_names <- names(edgeDataDefaults(g))
        e_names.len <- length(e_names)
        
        # edge attributes : list
        e_attr <- vector("list", e_names.len)
        
        names(e_attr) <- e_names
        
        if(length(g.edge.attr) > 0) {
            for(k in 1:e_names.len) {
                for(m in 1:length(g.edge.attr)) {
                    e_attr[[k]] <- c(e_attr[[k]], g.edge.attr[[m]][[k]])
                }
            }
        }
        
        # replicate original toCytoscape()
        attr_exists = FALSE
        e_names_len = 0
        ...
        for(i in 1:edge_count) {
            st = list(source=toString(edges.matrix[i,1]), target=toString(edges.matrix[i,2]))
